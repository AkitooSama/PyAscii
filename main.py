...
class Convertor:
    def __init__(
        ...
        self.font = ImageFont.truetype("C:\\Windows\\Fonts\\lucon.ttf", 10)
    ...
    def process_frame(self, iteration):
        text_filename = f"{self.text_frames_path}/Text_Frame{iteration}.txt"
        image_filename = f"{self.extract_frames_path}/frame{iteration}.png"
        ascii_image_filename = f"{self.ascii_frames_path}/ascii_frame{iteration}.png"
        ...
        with open(text_filename, "w") as text_file:
            ...
            output_image.save(ascii_image_filename)
            print(f"Ascii-Frame Converted- #{iteration}", end="\r")

    def ascii_converter(self):
        if not os.path.exists(self.ascii_frames_path):
            os.makedirs(self.ascii_frames_path, exist_ok=True)
        if not os.path.exists(self.text_frames_path):
            os.makedirs(self.text_frames_path, exist_ok=True)
        print("Directories Successfully Made!\n")

        items = os.listdir(self.extract_frames_path)
        max_iter = len(items)

        for i in range(max_iter):
            self.process_frame(i)
        # with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        #     executor.map(self.process_frame, range(max_iter))
